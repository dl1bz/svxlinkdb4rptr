var webAudioPeakMeter = (function () {
    ...
    var createMeter = function (domElement, meterNode, optionsOverrides) {
        ...
        createRainbow(
            meterElement, meterWidth, meterHeight,
            meterTop, tickWidth
        );
        ...
        for (var i = 0; i < channelCount; i++) {
            createChannelMask(
                meterElement, options.borderSize,
                meterTop, channelLeft, false
            );
            channelMasks[i] = createChannelMask(
                meterElement, channelWidth,
                meterTop, channelLeft,
                options.maskTransition
            );
            channelPeaks[i] = 0.0;
            channelPeakLabels[i] = createPeakLabel(
                meterElement, channelWidth,
                channelLeft
            );
